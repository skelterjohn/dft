...
func TestRename(t *testing.T) {
	...
	testCase(t, tc{
		...
		input:        `{"x":"z","y":"w"}`,
		args:         []string{"t:{.y=.x}"},
		expectedJSON: `{"x":"z","y":"z"}`,
	})
	testCase(t, tc{
		name:         "copy 2 to 1",
		input:        `[1,2,3,4]`,
		args:         []string{"t:{[1]=[2]}"},
		expectedJSON: `[1,3,3,4]`,
	})
	...
	testCase(t, tc{
		...
		input:        `[1,2,3,4]`,
		args:         []string{"t:{[6]=[2]}"},
		expectedJSON: `[1,2,3,4,null,null,3]`,
	})
}
...
func TestDeepTransforms(t *testing.T) {
	...
	testCase(t, tc{
		...
		input:        `[{"x":2}, {"x":4}]`,
		args:         []string{"t:[]{.y=.x}"},
		expectedJSON: `[{"x":2,"y":2},{"x":4,"y":4}]`,
	})
	testCase(t, tc{
		name:         "repeated field transform",
		input:        `{"x":[1,2,3], "y":[4,5,6]}`,
		args:         []string{"t:.(){[1]=[2]}"},
		expectedJSON: `{"x":[1,3,3], "y":[4,6,6]}`,
	})
